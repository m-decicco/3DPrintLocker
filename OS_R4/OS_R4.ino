...
bool storeBAT(char input){
  ...
  average = average/10;
  double batteryV = map(average,0,1024,0,5);
  ...
  if (myFile) {
    ...
    myFile.println(getTime()+", "+String(batteryV,2)+" V");
    myFile.close();
  } else {
    return false;
  }
...
void setup() {
  customKeypad.begin();
  RTC.begin();
  RTCTime startTime(3, Month::OCTOBER, 2023, 19, 26, 00, DayOfWeek::TUESDAY, SaveLight::SAVING_TIME_ACTIVE);
